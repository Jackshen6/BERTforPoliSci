...
# Custom functions necessary to use BERT to code data based on the Comparative Agendas Project typology.
...
def recode_data(speech, topicCode, sub_to_major = 0):
  ...
  - It recodes 23 to be 11 in order to eliminate gaps
  - It recodes 21 to be 0 if "other" category not present in order to eliminate gaps
  ...
  # recode 23 to 11
  topicCode = np.where(topicCode == 23, 11, topicCode)
  print('Recoded: 23 as 11')
  
  
  if 0 in topicCode:
      print('"Other" category present, 0 not recoded.')
  else:
      topicCode = np.where(topicCode == 21, 0, topicCode)
      print('"Other" category not present, 21 recoded to 0.')
...
def evaluation_metrics(predicted_categories, true_categories, accuracy = True, confusionMatrix = True, Precision = True, Recall = True, f1 = True, cohen = True):
  ...
  true_categories= true_categories.astype(str)
